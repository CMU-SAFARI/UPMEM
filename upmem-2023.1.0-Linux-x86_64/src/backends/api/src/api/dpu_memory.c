...
sg_xfer_rank_handler(struct dpu_set_t rank, __attribute((unused)) uint32_t rank_id, void *cb_args)
{
    ...
    DPU_FOREACH (rank, dpu, rank_dpu_index) {
        ...
        while (1) {
            ...
            block_exists = args->get_block_info->f(&binfo, dpu_index, block_index, args->get_block_info->args);
    ...
    dpu_sg_xfer_flags_t flags)
{
    ...
    bool is_default = ((flags & DPU_SG_XFER_DEFAULT) != 0);
    if (!(is_async ^ is_default))
        return DPU_ERR_INTERNAL;
    ...
    sg_xfer_cb_args->get_block_info = get_block_info;
    sg_xfer_cb_args->xfer = xfer;
    sg_xfer_cb_args->symbol_addr = symbol_addr;
    sg_xfer_cb_args->length = length;
    sg_xfer_cb_args->flags = flags;

    if ((status = dpu_callback(
             dpu_set, sg_xfer_rank_handler, sg_xfer_cb_args, is_default ? DPU_CALLBACK_DEFAULT : DPU_CALLBACK_ASYNC))
        != DPU_OK)
        return status;
