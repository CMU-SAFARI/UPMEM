...
static uint8_t **sg_buffer_pool_blocks_addr = NULL;
static uint32_t *sg_buffer_pool_blocks_length = NULL;
static bool sg_xfer_enabled = false;

static dpu_error_t
sg_buffer_pool_create(struct dpu_set_t set, uint32_t dpu_max_nr_blocks)
{
    sg_xfer_enabled = true;
...
dpu_error_t
init_satter_gather_transfer_buffer(uint32_t nr_dpus, const char *profile, struct dpu_set_t *dpu_set)
{
    dpu_error_t status = DPU_OK;
...
dpu_alloc(uint32_t nr_dpus, const char *profile, struct dpu_set_t *dpu_set)
{
    ...
    if ((status = init_satter_gather_transfer_buffer(nr_dpus, profile, dpu_set)) != DPU_OK) {
        goto error_free_ranks;
    }
...
dpu_free(struct dpu_set_t dpu_set)
{
    ...
    if ((status = sg_buffer_pool_free()) != DPU_OK) {
        return status;
    }
